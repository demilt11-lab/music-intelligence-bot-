...
function filterPreBreak(t: TalentScoutTrack, mode: TalentScoutMode): boolean {
  ...
  const combined =
    Number(t.spotifyStreamsLatest ?? 0) + Number(t.luminateStreamsLatest ?? 0)
...
function computeUgcEarlyScore(t: TalentScoutTrack): number {
  const ugcGrowth =
    growthScore(t.tiktokVelocity) * 0.35 + (t.tiktokScore ?? 0) * 0.25
...
function computeGeneralScore(t: TalentScoutTrack): number {
  ...
  return (
    growthScore(t.tiktokVelocity) * 0.3 +
    (t.tiktokScore ?? 0) * 0.2 +
    youtubeSignal +
    instagramSignal
  )
...
function baselineSizePenalty(
  ...
  const combined =
    Number(spotifyStreamsLatest ?? 0) + Number(luminateStreamsLatest ?? 0)
...
function buildActions(
  ...
  if (growthScore(t.tiktokVelocity) > 0.5) {
    actions.push({
      ...
      expectedImpact:
        'Sustain UGC velocity and widen creator base over next 7–14 days.',
    })
  }
  ...
  if (!t.spotifyStreamsLatest || Number(t.spotifyStreamsLatest) < 500_000) {
    actions.push({
      ...
      expectedImpact:
        'Convert UGC demand into 20–30% higher daily streams.',
    })
  }
  ...
  if (t.luminateSpinsLatest && Number(t.luminateSpinsLatest) > 0) {
    actions.push({
      type: 'radio_watch',
      description:
        'Monitor early radio adopters for potential targeted pushes.',
      expectedImpact:
        'Increase audience where spins are starting without overcommitting spend.',
    })
  }
  ...
  if (!actions.length && totalScore > 0) {
    actions.push({
      type: 'geo_focus',
      description:
        'Focus early marketing on top UGC geos where momentum is strongest.',
      expectedImpact:
        'Increase conversion from UGC to platform streams in those markets.',
    })
  }
