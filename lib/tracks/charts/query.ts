...
export async function queryTrackChartAppearances(
  ...
  return db.$queryRaw<RawChartRow[]>`
    SELECT
      ...
      -- External IDs as a JSON object {platform: [id, ...], ...}
      (
    ...
    LEFT JOIN track_albums tal ON tal.track_id = t.id AND tal.is_primary = TRUE
